...
if ($_REQUEST['s'] === $SESSIONHASH) {
	...
	$saved_zip_file = RATBACKUPSPATH.'zip/'. $timestamp .'_archive.zip';	
	
	$sourcePath = RATROOTPATH;
	...
	if (class_exists ( 'ZipArchive' , false)) {
		...
		foreach($iter as $element) {
		    ...
		    if ( strstr($dir, $RATADMIN.'/') || strstr($dir, 'backups/') ) {
		...
		$archiv->addFile(RATROOTPATH.'.htaccess', '.htaccess' );
		$archiv->addFile(RATROOTPATH.'RATconfig.php', 'RATconfig.php' );
