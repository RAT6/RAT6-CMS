...
if (isset($_GET['newfolder'])) {
	...
	if (file_exists($path.$cleanname) || $cleanname=='') {
		...
		if (defined('RATCHMOD')) { 
			$chmod_value = RATCHMOD; 
		} else {
			$chmod_value = 0755;
		}
		if (mkdir($path . $cleanname, $chmod_value)) {
			//create folder for thumbnails
			$thumbFolder = RATTHUMBNAILPATH.$subFolder.$cleanname;
				...
				foreach ($filesSorted as $upload) {
					...
					if ($upload['type'] == i18n_r('IMAGES') .' Images') {
						...
						if (file_exists('../data/thumbs/'.$thumbLink)) {
							$imRATrc='<img src="../data/thumbs/'. $thumbLink .'" />';
						} else {
							$imRATrc='<img src="inc/thumb.php?src='. $urlPath . $upload['name'] .'&amp;dest='. $thumbLink .'&amp;x=65&amp;f=1" />';
						}
						echo '<a href="'. $path . $upload['name'] .'" title="'. $upload['name'] .'" rel="facybox" >'.$imRATrc.'</a>';
					} else {
						$gallery = '';
						$controlpanel = '';
					...
					if ($isUnixHost && defined('RATDEBUG')) {
