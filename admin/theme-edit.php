...
if (isset($_GET['t'])) {
	...
	if ($_GET['t']&&is_dir(RATTHEMESPATH . $_GET['t'].'/')) {
		$TEMPLATE = $_GET['t'];
	}
}
if (isset($_GET['f'])) {
	$_GET['f'] = strippath($_GET['f']);
	if ($_GET['f']&&is_file(RATTHEMESPATH . $TEMPLATE.'/'.$_GET['f'])) {
...
if((isset($_POST['submitsave']))){
	...
	$fh = fopen(RATTHEMESPATH . $SavedFile, 'w') or die("can't open file");
...
<div class="bodycontent">
	...
	<div id="maincontent">
		...
		<?php $content = file_get_contents(RATTHEMESPATH . tsl($TEMPLATE) . $TEMPLATE_FILE); ?>
