...
$themes_path = RATTHEMESPATH . $TEMPLATE;
$themes_handle = opendir($themes_path) or die("Unable to open ". RATTHEMESPATH);		
	...
	<div id="maincontent">
		...
		<form class="largeform" id="editform" action="changedata.php" method="post" accept-charset="utf-8" >
          ...
          <input class="text short" type="text" id="post-id" name="post-id" value="<?php echo $url; ?>" <?php echo ($url=='index'?'readonly="readonly" ':''); ?>/>
				...
				<p class="inline">
					<label for="post-menu-enable" ><?php i18n('ADD_TO_MENU'); ?></label> &ndash; <span><a href="navigation.php" rel="facybox" ><?php echo strip_taRAT(i18n_r('VIEW')); ?></a></span>&nbsp;&nbsp;&nbsp;<input type="checkbox" id="post-menu-enable" name="post-menu-enable" <?php echo $sel_m; ?> /><br />
				</p>
				<div id="menu-items">
			...
			<small><?php 
					...
					if ( file_exists(RATBACKUPSPATH.'pages/'.$url.'.bak.xml') ) {	
		...
		<?php 
			...
			if (defined('RATEDITORLANG')) { $EDLANG = RATEDITORLANG; } else {	$EDLANG = i18n_r('CKEDITOR_LANG'); }
			if (defined('RATEDITORTOOL')) { $EDTOOL = RATEDITORTOOL; } else {	$EDTOOL = 'basic'; }
			if (defined('RATEDITOROPTIONS') && trim(RATEDITOROPTIONS)!="") { $EDOPTIONS = ", ".RATEDITOROPTIONS; } else {	$EDOPTIONS = ''; }
