...
*/
if (file_exists(RATPLUGINPATH)){
	$pluginfiles = getFiles(RATPLUGINPATH);
} 
...
if (!file_exists(RATDATAOTHERPATH."plugins.xml")){
   create_pluginsxml();
} 
...
foreach ($live_plugins as $file=>$en) {
  $pluginsLoaded=true;
  if (file_exists(RATPLUGINPATH . $file)){
  	require_once(RATPLUGINPATH . $file);
  }
}
...
function read_pluginsxml(){
  ...
  $data = getXML(RATDATAOTHERPATH . "plugins.xml");
...
function create_pluginsxml(){
  ...
  XMLsave($xml, RATDATAOTHERPATH."plugins.xml");
  read_pluginsxml();
}
 ...
 * @param array $arRAT
 */
function add_action($hook_name, $added_function, $arRAT = array()) {
	global $plugins;
	global $live_plugins; 
...
function exec_action($a) {
	...
	foreach ($plugins as $hook)	{
		if ($hook['hook'] == $a) {
			call_user_func_array($hook['function'], $hook['arRAT']);
		}
	}
}
