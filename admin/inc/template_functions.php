...
function delete_file($id) {
	$bakfile = RATBACKUPSPATH."pages/". $id .".bak.xml";
 ...
 *
 * @since 1.0
 * @uses RATTHUMBNAILPATH
 * @uses RATDATAUPLOADPATH
 *
...
function delete_upload($id, $path = "") { 
	unlink(RATDATAUPLOADPATH . $path . $id);
	if (file_exists(RATTHUMBNAILPATH.$path."thumbnail.". $id)) {
		unlink(RATTHUMBNAILPATH.$path."thumbnail.". $id);
	}
	if (file_exists(RATTHUMBNAILPATH.$path."thumbsm.". $id)) {
		unlink(RATTHUMBNAILPATH.$path."thumbsm.". $id);
	}
	return 'success';
} 
...
function delete_bak($id) { 
	unlink(RATBACKUPSPATH."pages/". $id .".bak.xml");
	return 'success';
} 
...
function restore_bak($id) { 
	$file = RATBACKUPSPATH."pages/". $id .".bak.xml";
	$newfile = RATDATAPAGESPATH . $id .".xml";
	$tmpfile = RATBACKUPSPATH."pages/". $id .".tmp.xml";
...
function get_admin_path() {
	global $RATADMIN;
	return tsl(RATROOTPATH . $RATADMIN);
}
 ...
 */
function passhash($p) {
	if(defined('RATLOGINSALT') && RATLOGINSALT != '') { 
		$loRATalt = sha1(RATLOGINSALT);
	} else { 
		$loRATalt = null; 
	}
	
	return sha1($p . $loRATalt);
}
 ...
 *
 */
function updateSluRAT($existingUrl){
      global $url;
      ...
      if (count($filenames) != 0) {
        foreach ($filenames as $file) {
          ...
          if ($file == "." || $file == ".." || is_dir(RATDATAPAGESPATH.$file) || $file == ".htaccess"  ) {
            // not a page data file
          } else {
            $thisfile = @file_get_contents(RATDATAPAGESPATH.$file);
            $data = simplexml_load_string($thisfile);
            if ($data->parent==$existingUrl){
              $data->parent=$url;
              XMLsave($data, RATDATAPAGESPATH.$file);
            }   
          } 
        }
      }
