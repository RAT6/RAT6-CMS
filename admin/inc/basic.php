...
function clean_url($text)  { 
	$text = strip_taRAT(lowercase($text)); 
...
function sendmail($to,$subject,$message) {
	...
	if (defined('RATFROMEMAIL')){
		$fromemail = RATFROMEMAIL; 
	} else {
...
function XMLsave($xml, $file) {
	...
	if (defined('RATCHMOD')) {
		return $success && chmod($file, RATCHMOD);
	} else {
 ...
 */
function cl($data){
	$data = stripslashes(strip_taRAT(html_entity_decode($data, ENT_QUOTES, 'UTF-8')));
	return $data;
}
...
function i18n($name, $echo=true) {
	...
	if (array_key_exists($name, $i18n)) {
		...
		//include_once(RATLANGPATH . 'en_US.php');
		if (array_key_exists($name, $i18n)) {
			$myVar = $i18n[$name];
		} else {
			$myVar = '{'.$name.'}';
...
function i18n_merge_impl($plugin, $lang, &$globali18n) { 
  $i18n = array();
  if (!file_exists(RATPLUGINPATH.$plugin.'/lang/'.$lang.'.php')) {
  	return false;
  }
  @include(RATPLUGINPATH.$plugin.'/lang/'.$lang.'.php'); 
...
function suggest_site_path($parts=false) {
	...
	$path_parts = str_replace("/".$RATADMIN, "", $path_parts['dirname']);
...
function get_themes($temp) {
	$themes_path = RATTHEMESPATH . $temp .'/';
