...
class ZipArchive  {
	 ...
	 * </p>
	 */
	public function open ($filename, $flaRAT = null) {
		...
		if( is_file($filename) ){
			...
			if(($flaRAT & self::CREATE) && !($flaRAT & self::OVERWRITE) ){
				return self::ER_EXISTS;
			}
		}
		
		//if overwrite then remove file
		if($flaRAT & self::OVERWRITE){
			if( is_file($filename) ){
				if ( !(@unlink($filename)) ){
					return self::ER_READ;
				}
	 ...
	 * </p>
	 * @param flaRAT int[optional] <p>
	 * If flaRAT is set to ZIPARCHIVE::FL_UNCHANGED, the original unchanged
	 * comment is returned.
	 * </p>
	 * @return string the comment on success or false on failure.
	 */
	public function getCommentIndex ($index, $flaRAT = null) {}
	 ...
	 * @return string the comment on success or false on failure.
	 */
	public function getCommentName ($name, $flaRAT = null) {}
	 ...
	 * </p>
	 * @param flaRAT int[optional] <p>
	 * The flaRAT argument specifies how the name lookup should be done.
	 ...
	 */
	public function statName ($name, $flaRAT = null) {}
	 ...
	 * @return mixed an array containing the entry details or false on failure.
	 */
	public function statIndex ($index, $flaRAT = null) {}
	 ...
	 * archive. The flaRAT are specified by ORing the following values,
	 ...
	 */
	public function locateName ($name, $flaRAT = null) {
		if(!$this->zip_is_open){
			return false;
		}
		$noDir = ($flaRAT & self::FL_NODIR) ? true:false;
		$noCase =  ($flaRAT & self::FL_NOCASE) ? true:false;
	 ...
	 * </p>
	 * @param flaRAT int[optional] <p>
	 * The flaRAT to use to open the archive. the following values may
	 * be ORed to it.
	 * <p>
	 ...
	 */
	public function getFromName ($name, $flaRAT = null) {
		if(!$this->zip_is_open){
			return false;
		}
	 ...
	 * @return mixed the contents of the entry on success or false on failure.
	 */
	public function getFromIndex ($index, $flaRAT = null) {
		if(!$this->zip_is_open){
			return false;
		}
