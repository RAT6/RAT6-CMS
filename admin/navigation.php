...
login_cookie_check();
	...
	$dir_handle = opendir(RATDATAPAGESPATH) or die("Unable to open ". RATDATAPAGESPATH);
	$filenames = array();
		...
		foreach ($filenames as $file) {
			if ($file == "." || $file == ".." || is_dir(RATDATAPAGESPATH . $file) || $file == ".htaccess"  ) {
				// not a page data file
					...
					$data = getXML(RATDATAPAGESPATH . $file);
				if ($data->private != 'Y') {
...
if(!defined('IN_RAT')){ die('you cannot load this page directly.'); }
global $LANG;
	...
	if (count($pagesSorted) != 0) { 
		...
		foreach ($pagesSorted as $page) {
			...
			if ($page['menuStatus'] != '') { 
				...
				<td>'. $page['title'] .'</td>
				<td><a href="'.find_url($page['url'], $page['parent']).'" target="_blank" >'.strip_taRAT(i18n_r('VIEW')).'</a></td>
				<td><a href="edit.php?id='.$page['url'].'" target="_blank" >'.strip_taRAT(i18n_r('EDIT')).'</a></td>
				</tr>';
			}
		}
		echo '</table>';
