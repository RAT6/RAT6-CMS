...
$innovation_file=RATDATAOTHERPATH .'InnovationSettinRAT.xml';

# register plugin
register_plugin(
	$thisfile, 													# ID of plugin, should be filename minus php
	'Innovation Theme SettinRAT', 									# Title of plugin
...
# hooks
add_action('theme-sidebar','createSideMenu',array($thisfile,'Innovation Theme SettinRAT')); 
...
function innovation_show() {
	...
	if (isset($_POST['submit'])) {
		...
		if (!$error) {
			...
			if (! $xml->asXML($innovation_file)) {
				...
				$twitter = $x->twitter;
				$linkedin = $x->linkedin;
				$success = i18n_r('SETTINRAT_UPDATED');
			}
		}
	}
	
	?>
	<h3>Innovation Theme SettinRAT</h3>
	...
	<form method="post" action="<?php	echo $_SERVER ['REQUEST_URI']?>">
		...
		<p><input type="submit" id="submit" class="submit" value="<?php i18n('BTN_SAVESETTINRAT'); ?>" name="submit" /></p>
	</form>
